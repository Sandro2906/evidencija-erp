...
export async function addPurchase(formData) {
  ...
  const entry_date = formData.get('entry_date') || new Date().toISOString();
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO material_entries(material_id, supplier_id, quantity, price_per_unit, entry_date) VALUES($1, $2, $3, $4, $5)',
      [material_id, supplier_id, quantity, price_per_unit, entry_date]
    );
